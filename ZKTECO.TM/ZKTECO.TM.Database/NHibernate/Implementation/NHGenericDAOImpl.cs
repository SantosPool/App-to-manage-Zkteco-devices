...
namespace ZKTECO.TM.Database.NHibernate.Implementation
{
    public class NHGenericDAOImpl<T, PK> : INHGenericDAO<T, PK> where T : IGenericEntity<PK>
    {
        ...
        public virtual Page<T> PageItems(int page, int pageSize, string search)
        {
            using (Session)
            {
                using (Transaction = Session.BeginTransaction())
                {
                    try
                    {
                        ...
                        var items = c.List<T>();
                        var count = Session.QueryOver<Type>()
                            .And(Restrictions.Like(NAME, "%" + search + "%"))
                            .RowCountInt64();
                        Page<T> pageResult = new Page<T>(items, count, page, pageSize);
                        return pageResult;
                    }
                    catch (Exception e)
                    {
